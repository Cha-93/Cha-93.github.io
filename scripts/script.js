...
function hideAllRecipe(){
    let allRecipe = []
    recipes.forEach((recipe) => {
        allRecipe.push(recipe.id);
    })
    allRecipe.forEach(id => {
        let recipeCard = document.getElementById(`recipeId${id}`)
        if(recipeCard){
...
function setSearchInput(){
    ...
    let searchFilter = []; // Contient les ids qui correspondent à la recherche (par le nom)

    //  Recherche par nom
    recipes.forEach((recipe) => {
        let name = recipe.name.toLowerCase() 
        if (name.includes(searchInput) === true) {
            searchFilter.push(recipe.id)
        }
    })
...
let activeFiltersElement = document.getElementById('active-filters');
...
function showFilteredRecipes(filteredIds) {
...
// algorithme : version boucles natives

function filterRecipes() {
    let filteredIds = [];
    for (let i = 0; i < recipes.length; i++) {
        filteredIds.push(recipes[i].id);
    }
    ...
    if (selectedIngredients.length > 0) {
        ...
        for (let i = 0; i < filteredIds.length; i++) {
            ...
            for (let k = 0; k < selectedIngredients.length; k++) {
                let ingredient = selectedIngredients[k].toLowerCase();
                let ingredientFound = false;
                for (let l = 0; l < recipe.ingredients.length; l++) {
                    if (recipe.ingredients[l].ingredient.toLowerCase() === ingredient) {
    ...
    if (selectedAppliances.length > 0) {
        ...
        for (let i = 0; i < filteredIds.length; i++) {
            ...
            for (let j = 0; j < selectedAppliances.length; j++) {
                if (recipe.appliance.toLowerCase() === selectedAppliances[j].toLowerCase()) {
                    newFilteredIds.push(id);
                    break;
                }
            }
    ...
    if (selectedUstensils.length > 0) {
        ...
        for (let i = 0; i < filteredIds.length; i++) {
            ...
            for (let k = 0; k < selectedUstensils.length; k++) {
                let ustensil = selectedUstensils[k].toLowerCase();
                let ustensilFound = false;
                for (let l = 0; l < recipe.ustensils.length; l++) {
                    if (recipe.ustensils[l].toLowerCase() === ustensil) {
                        ustensilFound = true;
                        break;
                    }
                }
    ...
    hideAllRecipe();
    showFilteredRecipes(filteredIds);
    updateActiveFilters();
}
...
function toggleFilter(type, value) {
    if (type === 'ingredient') {
        toggleArrayItem(selectedIngredients, value);
    } else if (type === 'appliance') {
        toggleArrayItem(selectedAppliances, value);
    } else if (type === 'ustensil') {
        toggleArrayItem(selectedUstensils, value);
    }
    filterRecipes();
}
...
function updateActiveFilters() {
    ...
    selectedIngredients.forEach(function(ingredient) {
        ...
        filterDiv.innerText = ingredient;
        filterDiv.onclick = function() { toggleFilter('ingredient', ingredient); };
        activeFiltersElement.appendChild(filterDiv);
    });

    selectedAppliances.forEach(function(appliance) {
        let filterDiv = document.createElement('div');
        filterDiv.className = 'active-filter';
        filterDiv.innerText = appliance;
        filterDiv.onclick = function() { toggleFilter('appliance', appliance); };
        activeFiltersElement.appendChild(filterDiv);
    });

    selectedUstensils.forEach(function(ustensil) {
        let filterDiv = document.createElement('div');
        filterDiv.className = 'active-filter';
        filterDiv.innerText = ustensil;
        filterDiv.onclick = function() { toggleFilter('ustensil', ustensil); };
        activeFiltersElement.appendChild(filterDiv);
    });
}
...
{/* <div class="recipe" id="recipeId3">
    <div class="recipe-header">
        <span class="time">50min</span>
        <img src="../assets/images/recipes/Recette08.jpg"/>
    </div>
    
    <div class="recipe-content">
        <h3 class="recipe-title">Poulet coco réunionnais</h3>
        <div class="recipe-description-container">
            <h4 class="recipe-description-title">RECETTE</h4>
            <p>
                Hachez les noix grossièrement. Faire fondre le chocolat avec
                le beurre. Mélanger les oeufs et le sucre et mélanger au chocolat.
                Ajouter la farine. Mélanger afin d'avoir quelque chose d'homogène.
            </p>
        </div>  
        <div class="recipe-ingredients-container">
            <h4 class="recipe-ingredients-title">INGRÉDIENTS</h4>
            <div class="recipe-allIngredients">
                <div>
                    <label class="ingredient-name">Noix</label>
                    <label class="ingredient-value">180grammes</label>
                </div>
                <div>
                    <label class="ingredient-name">Beurre</label>
                    <label class="ingredient-value">120grammes</label>
                </div>
                <div>
                    <label class="ingredient-name">Sucre en Poudre</label>
                    <label class="ingredient-value">110grammes</label>
                </div>
                <div>
                    <label class="ingredient-name">Noix</label>
                    <label class="ingredient-value">180grammes</label>
...
recipes.forEach(recipe => {
    ...
    divRecipe.setAttribute('id', `recipeId${recipe.id}`);
    ...
    let recipeTimeText = document.createTextNode(recipe.time + 'min');
    ...
    recipeContent.setAttribute('class', 'recipe-content');
    
    let recipeTitle = document.createElement("h3");
    recipeTitle.setAttribute('class', 'recipe-title');
    let recipeTitleText = document.createTextNode(recipe.name);
    recipeTitle.appendChild(recipeTitleText);
    recipeContent.appendChild(recipeTitle);

    let recipeDescContainer = document.createElement("div");
    recipeDescContainer.setAttribute('class', 'recipe-description-container');

    let recipeDescTitle = document.createElement("h4");
    recipeDescTitle.setAttribute('class', 'recipe-description-title');
    let recipeDescTitleText = document.createTextNode('RECETTE');
    recipeDescTitle.appendChild(recipeDescTitleText);
    recipeDescContainer.appendChild(recipeDescTitle);

    let recipeDesc = document.createElement("p");
    let recipeDescText = document.createTextNode(recipe.description);
    recipeDesc.appendChild(recipeDescText);
    recipeDescContainer.appendChild(recipeDesc);

    recipeContent.appendChild(recipeDescContainer)
    // --------------

    
    // recipe-ingredients-container
    let recipeIngreContainer = document.createElement("div");
    recipeIngreContainer.setAttribute('class', 'recipe-ingredients-container');

    let recipeIngreTitle = document.createElement("h4");
    recipeIngreTitle.setAttribute('class', 'recipe-ingredients-title');
    let recipeIngreTitleText = document.createTextNode('INGRÉDIENTS');
    recipeIngreTitle.appendChild(recipeIngreTitleText);
    recipeIngreContainer.appendChild(recipeIngreTitle)

    let recipeAllIngr = document.createElement("div");
    recipeAllIngr.setAttribute('class', 'recipe-allIngredients');
    
    recipe.ingredients.forEach((ingr) => {
        let simpleDiv = document.createElement("div");

        let ingrName = document.createElement('label');
        ingrName.setAttribute('class', 'ingredient-name');
        let ingrNameText = document.createTextNode(ingr.ingredient)
        ingrName.appendChild(ingrNameText);
        simpleDiv.appendChild(ingrName);
        ...
        if (ingr.unit !== undefined){
            ...
            ingrValueText = document.createTextNode(ingr.quantity);
        }
        ingrValue.appendChild(ingrValueText);
        simpleDiv.appendChild(ingrValue);

        recipeAllIngr.appendChild(simpleDiv);
    });

    recipeIngreContainer.appendChild(recipeAllIngr);

    recipeContent.appendChild(recipeIngreContainer);
    // ----------------------------

    divRecipe.appendChild(recipeHeader)
    divRecipe.appendChild(recipeContent)

    recipesList.appendChild(divRecipe)
    ...
    // recipeImg.setAttribute('src', './assets/img.jpg')
    // // <img src="./assets/img.jpg" class="recipeImage"></img>
    // recipeImg.setAttribute('class', "recipeImage")

    // // Pour Descrition :
    // // <p></p>
    // let recipeDesc = document.createElement("p");
    // // <p class="recipeDesc"></p>
    // recipeDesc.setAttribute('class', "recipeDesc")
    // // <p class="recipeDesc">...</p>
    // let DescritptionText = document.createTextNode(recipe.description)
    // recipeDesc.append(DescritptionText);
    
    // //Pour appliance
    // let recipeAppliance = document.createElement("p");
    // recipeAppliance.setAttribute('class',"recipeAppliance")
    // let ApplianceText = document.createTextNode(recipe.appliance)
    // recipeAppliance.append(ApplianceText);
    // recipeAppliance.style.display = "none";
    
    // //Pour ustensils
    // let recipeUstensils = document.createElement("p");
    // recipeUstensils.setAttribute('class',"recipeUstensils")
    // let UstensilsText = document.createTextNode(recipe.ustensils)
    // recipeUstensils.append(UstensilsText)
    // recipeUstensils.style.display = "none"
    
    // //Pour ingrédients
    // let recipeIngredients = document.createElement("p");
    // recipeIngredients.setAttribute('class',"recipeIngredients")
    // let IngredientsText = document.createTextNode(recipe.ingredients)
    // recipeIngredients.append(IngredientsText)
    
    
    // // Ajout des eléments dans le la div recipesList
    // recipesList.appendChild(recipeDesc)
    // recipesList.appendChild(recipeAppliance)
    // recipesList.appendChild(recipeUstensils)
    // recipesList.appendChild(recipeIngredients)
